...
public class L1CookingIngredient {
	...
	private L1CookingIngredient(int id, int cookingRecipeId, int itemId, int amount) {
		super();
		this._id = id;
	...
	private static class Factory implements EntityFactory<L1CookingIngredient> {
		@Override
		public L1CookingIngredient fromResultSet(ResultSet rs) throws SQLException {
			int id = rs.getInt("id");
	...
	public static List<L1CookingIngredient> findByCookingRecipeId(int cookingRecipeId) {
		return L1QueryUtil.selectAll(new Factory(), "SELECT * FROM cooking_ingredients WHERE cooking_recipe_id = ?", cookingRecipeId);
	}
}