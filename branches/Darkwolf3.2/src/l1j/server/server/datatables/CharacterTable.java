...
public class CharacterTable {
	...
	public void storeNewCharacter(L1PcInstance pc) throws Exception {
		synchronized (pc) {
			...
			_log.finest("Stored New Character Info.");
		}
	}
	...
	public void deleteCharacter(String accountName, String charName) throws Exception {
		...
		_log.finest("Deleting Character Info.");
	}
	...
	public L1PcInstance loadCharacter(String charName) throws Exception {
		...
		try {
			...
			_log.finest("Loading Character Info : " + pc.getName());
		} catch (Exception e) {
	...
	public static void updateOnlineStatus(L1PcInstance pc) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("UPDATE characters SET online_status=? " +
					"WHERE id=?");
	...
	public static void updatePartnerId(int targetId, int partnerId) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("UPDATE characters SET partner_id=? " +
					"WHERE id=?");
	...
	public static void saveCharStatus(L1PcInstance pc) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con
					.prepareStatement("UPDATE characters SET original_str= ?"
							+ ", original_con= ?, original_dex= ?, original_cha= ?"
							+ ", original_int= ?, original_wis= ?"
							+ " WHERE id=?");
	...
	public static boolean doesCharNameExist(String name) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("SELECT name FROM characters " +
					"WHERE name=?");
			pstm.setString(1, name);
			rs = pstm.executeQuery();
			result = rs.next();
		} catch (SQLException e) {
			_log.warning("Could not check excisting Character Name:" + e.getMessage());
		} finally {
			SQLUtil.close(rs);
			SQLUtil.close(pstm);
			SQLUtil.close(con);
