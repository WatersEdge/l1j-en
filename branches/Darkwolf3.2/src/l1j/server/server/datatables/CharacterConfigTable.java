...
public class CharacterConfigTable {
	...
	public void storeCharacterConfig(int objectId, int length, byte[] data) {
		...
		try {
			...
			pstm = con.prepareStatement("INSERT INTO character_config SET " +
					"object_id=?, length=?, data=?");
	...
	public void updateCharacterConfig(int objectId, int length, byte[] data) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("UPDATE character_config SET length=?, " +
					"data=? WHERE object_id=?");
	...
	public void deleteCharacterConfig(int objectId) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("DELETE FROM character_config " +
					"WHERE object_id=?");
			pstm.setInt(1, objectId);
			pstm.execute();
		} catch (SQLException e) {
	...
	public int countCharacterConfig(int objectId) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("SELECT count(*) as cnt FROM " +
					"character_config WHERE object_id=?");
			pstm.setInt(1, objectId);
