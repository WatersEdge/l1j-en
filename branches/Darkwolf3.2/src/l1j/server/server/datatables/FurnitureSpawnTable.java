...
public class FurnitureSpawnTable {
	...
	public void insertFurniture(L1FurnitureInstance furniture) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("INSERT INTO spawnlist_furniture SET " +
					"item_obj_id=?, npcid=?, locx=?, locy=?, mapid=?");
	...
	public void deleteFurniture(L1FurnitureInstance furniture) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("DELETE FROM spawnlist_furniture " +
					"WHERE item_obj_id=?");
			pstm.setInt(1, furniture.getItemObjId());
			pstm.execute();
		} catch (SQLException e) {
